...
const ActiveProfileTabContext = createContext<ProfileTabType>("profil");

export const useActiveProfileTab = () => useContext(ActiveProfileTabContext);
...
export const getTabTitle = (tab: ProfileTabType): { title: string; subtitle: string } => {
  const titles = {
    ...
    sante: { title: "Ma Santé", subtitle: "Allergies et pathologies" },
    stocks: { title: "Mes Médicaments", subtitle: "Suivi des médicaments" },
  };
  return titles[tab];
};
...
export function ProfileTabs({ activeTab, onTabChange, children }: ProfileTabsProps) {
  return (
    <ActiveProfileTabContext.Provider value={activeTab}>
