...
_Copies minimales pour offline:_
...
_Personnalisation utilisateur:_
...
// ============================================
export async function getMedicationByCIS(
  cis: string,
): Promise<MedicationReference> {
  ...
  const stale =
    ref &&
    Date.now() - new Date(ref.lastsyncedat).getTime() > 30 * 24 * 3600 * 1000;
...
serve(async (req) => {
  try {
    ...
    return new Response(JSON.stringify({ synced: stale.length }), {
      headers: { "Content-Type": "application/json" },
    });
  } catch (error) {
    console.error(error);
